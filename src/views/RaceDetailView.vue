...
const isThisEventLive = computed(
  () => isLive.value && currentEvent.value?.id === eventId,
);

const { results, grid, isLoading, error } = useRaceDetail(
  eventId,
  isThisEventLive.value,
);
...
const currentEventMeta = computed(
  () => events.value.find((e) => e.id === eventId) ?? null,
);
const hasNoRaceData = computed(
  () =>
    ...
    sortedGrid.value.length === 0,
);

function positionLabel(position: number | null | undefined): string {
  return position == null ? "DNF" : String(position);
}
  ...
  <div class="space-y-8">
    ...
    <template v-else>
      <section v-if="sortedResults.length > 0">
        <h2
          class="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-3"
        >
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <span class="text-gray-900 dark:text-white font-medium">{{
              result.rider.full_name
            }}</span>
            <span
              v-if="result.team"
              class="text-gray-400 dark:text-gray-500 text-xs mr-auto"
              >{{ result.team.name }}</span
            >
          </div>
        </div>
      </section>
      ...
      <section v-if="sortedGrid.length > 0">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div
            v-for="entry in sortedGrid"
            :key="entry.rider.number"
            class="card p-3 text-center"
          >
            <p class="text-gray-400 dark:text-gray-500 text-xs mb-1">
              P{{ entry.position }}
            </p>
